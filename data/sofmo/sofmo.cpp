...
void sofmo::end(void)
{
	...
	sofmo::activeObject = 0;
}

void sofmo::demodulate(void)
{
	uint8_t t = TCNT2;
	uint8_t diff;
...
{
	sofmo::activeObject->demodulate();
}

void sofmo::recv(void)
{
	uint8_t high;
...
size_t sofmo::write(const uint8_t *buffer, size_t size)
{
...
size_t sofmo::write(uint8_t data)
{
    return write(&data, 1);
}
